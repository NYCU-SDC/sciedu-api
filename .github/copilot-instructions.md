...
- **API Specification**: **TypeSpec (TSP)** - The single source of truth for API definitions. _(Note: This does not apply to the `sciedu-backend` project. This convention is specifically for projects like `sciedu-api` / `core-system-api`.)_
...
- **Source of Truth**: Always refer to `.tsp` files in the `service/` directory for models and operation signatures. _(Note: Skip this requirement for the `sciedu-backend` project)._
...
1. **Sync Specs**: Review the corresponding `.tsp` file (source of truth). _(Note: Skip this for `sciedu-backend`)_.
